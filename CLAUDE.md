...
### 🤝 与李兆杰增量一致性核验（06-03，重要）
- 李把 INFRA.docx 里的增量表(City center/Core competition/Transport hub/Tourist 4类×3表=赛事日/非赛事日/总和)**自己编的**(非严格计算)。
- **区域总增量一致✓**:你 +9.7% = 他 "All zones +9.71%";且他 All zones 数 = 你的**精确×2**,3/4类别 background 也=你"进+出(行+列和)"精确相等 → **证明同一套矩阵,他用"进+出"口径、你用"到达量(列和)"口径**。
- **逐区增幅对不上**(且本就对不上):你=到达量+**赛事日(增量÷20浓缩)**,他=进+出+类别+整体/日型 → **是不同的量**,非谁错。
